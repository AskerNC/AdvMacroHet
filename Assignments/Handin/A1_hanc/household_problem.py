...
def solve_hh_backwards(par,z_trans,r,w,taua,taul,theta,xh,vbeg_a_plus,vbeg_a,a,c,ell,u):
    ...
    for i_fix in range(par.Nfix):
        ...
        for i_z in range(par.Nz):
            ...
            fac = (theta* wt**theta* (1-taul) / ( par.varphi_grid[i_fix] *xh_theta )) **(1/ (par.nu-theta+1) )
            
            # b. use FOCs
            c_endo = ( par.beta * vbeg_a_plus[i_fix,i_z] ) **( -1/ par.sigma )
            ell_endo = fac *( c_endo ) **( - par.sigma /( par.nu-theta+1) )
            
            # c. interpolation
            m_endo = c_endo + par.a_grid - (1-taul)*((wt * ell_endo)**(theta)/xh_theta )
            
            interp_1d_vec( m_endo , c_endo , m_exo , c[i_fix,i_z] )

            interp_1d_vec( m_endo , ell_endo , m_exo , ell[i_fix,i_z] )
            
            a[i_fix,i_z,:] = m_exo +  (1-taul)*((wt * ell[i_fix,i_z])**(theta)/xh_theta ) - c[i_fix,i_z]
            ...
            for i_a in range( par.Na ):
                if a[i_fix,i_z,i_a ] < 0.0:
                    ...
                    it = 0
                    while True :
                        ci = (1+ rt ) * par.a_grid[i_a] + (1-taul)*((wt * elli)**(theta)/xh_theta )
                        error = elli - fac * ci **( - par.sigma / (par.nu-theta+1) )
                        if np.abs( error ) < par.tol_ell :
                            break
                        else :
                            derror = 1 - fac *( - par.sigma / (par.nu-theta+1) ) * ci**( - par.sigma / (par.nu-theta+1)-1) * theta* wt**theta* (1-taul)/xh_theta * elli**(theta-1) 
